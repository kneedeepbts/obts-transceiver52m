...
void XCCHL1Decoder::decrypt()
{
	...
	for (int i = 0; i < 4; i++) {
		...
		if (mEncryptionAlgorithm == 1) {
            ...
            // FIXME: Still have to make the A53 Library into an actual library so it can be linked.
            //        Putting the A51 call here to make the code compile.  This may end up not being
            //        needed for the transceiver code, but this is a dependency mess, so don't know yet.
...
void MSPhysReportInfo::processPhysInfo(const RxBurst &inBurst)
{
    ...
    unsigned count = min((int)mReportCount,(int)8);
	...
	for (int qi=0,B=0; B<4; B++) {
		...
		if (mEncrypted == ENCRYPT_YES) {
			...
			if (mEncryptionAlgorithm == 1) {
                ...
                // FIXME: Using the A51 instead of A53 as a placeholder until A53 can be made into a proper library
...
void TCHFACCHL1Encoder::dispatch()
{
    ...
    int p = currentFACCH ? 0.0 * (float)0xFFFFFF : 0;
	...
	for (int B=0; B<4; B++) {
		...
		if (mEncrypted == ENCRYPT_YES) {
			...
			if (mEncryptionAlgorithm == 1) {
                ...
                // FIXME: Using A51 as a placeholder for A53 until library
				// A53_GSM(kc, 64, count, block1, block2);
                A51_GSM(kc, 64, count, block1, block2);
			} else {
				devassert(0);
