...
//    vector from another the options were:
//     * clone - allocate a new vector from memory
//     * alias - make the new vector point into the memory of the original vector
//     * shift - the new Vector steals the memory ownership from the original vector
//
...
//    class Vector:
//       The copy-constructor and assignment operators did a clone for the const
//       case and a shift for the non-const case.  This is really horrible.  The
//       segment methods were identical for const and non-const cases, always
//       returning an alias.  This also resulted in zillions of redundant
//       mallocs and copies throughout the code base.
//    class BitVector:
//       Copy-constructor:
//          BitVector did not have any copy-constructors, and I think the intent
//          was that it would have the same behavior as Vector, but that is not
//          how C++ works: with no copy-constructor the default copy-constructor
//          uses only the const case, so only the const Vector copy-constructor
//          was used.  Therefore it always cloned, and the code base relied
//          heavily on the "Return Value Optimization" to work at all.
//       Assignment operator:
//          BitVector did not have one, so C++ makes a default one that calls
//          Vector::operator=() as a side effect, which did a clone; not sure if
//          there was a non-const version and no longer care.
//       segment methods:
//          The non-const segment() returned an alias, and the const segment()
//          returned a clone.  I think the intent was that the behavior should
//          be the same as Vector, but there was a conversion of the result of
//          the const segment() method from Vector to BitVector which caused the
//          Vector copy-constructor to be (inadvertently) invoked, resulting in
//          the const version of the segment method returning a clone.
//
// What the behavior is now:
//    VectorBase:
//       There is a new VectorBase class that has only the common methods and
//       extremely basic constructors.  The VectorBase class MUST NOT CONTAIN:
//        * copy constructors
//        * non-trivial constructors called from derived classes
//        * or any method that returns a VectorBase type object.
//       Why?  Because any of the above when used in derived classes can cause
//       copy-constructor invocation, often surprisingly, obfuscating the code.
//       Each derived class must provide its own: copy-constructors and
//       segment() and related methods, since we do not want to inadvertently
//       invoke a copy-constructor to convert the segment() result from
//       VectorBase to the derived type.
//    BitVector:
//       The BitVector copy-constructor and assignment operator (inherited from
//       VectorBase) paradigm is: if the copied Vector owned memory, perform a
//       clone so the new vector owns memory also, otherwise just do a simple
//       copy, which is another alias.  This isn't perfect but works every place
//       in our code base and easier to use than the previous paradigm.  The
//       segment method always returns an alias.  If you want a clone of a
//       segment, use cloneSegment(), which replaces the previous
//       `const segment(...) const` method.  Note that the semantics of
//       cloneSegment still rely on the Return Value Optimization.  Oh well, we
//       should use refcnts.
//    Vector:
//       I left Vector alone (except for rearrangement to separate out
//       VectorBase.)  Vector should just not be used.
//    SoftVector:
//       SoftVector and signalVector should be updated similar to BitVector, but
...
public:
    ...
    [[nodiscard]] size_t bytes() const { return this->size() * sizeof(T); }
...
public:
    ...
    explicit Vector(size_t wSize = 0) { this->resize(wSize); }
    ...
    /** Return an alias to a segment of this Vector. */
    Vector<T> segment(size_t start, size_t span) const {
        T * wStart = this->mStart + start;
        T * wEnd = wStart + span;
        assert(wEnd <= this->mEnd);
        return Vector<T>(NULL, wStart, wEnd);
    ...
    Vector<T> head(size_t span) const { return segment(0, span); }

    Vector<T> tail(size_t start) { return segment(start, this->size() - start); }

    Vector<T> tail(size_t start) const { return segment(start, this->size() - start); }
