...
//		dopipe = 1;
//		if (pipe(pipefd) == -1) {
//			SPDLOG_ERROR("could not create pipe: {}", strerror(errno));
//			return -1;
//		}
//	}
//	int pid = fork();
//	if (pid == -1) {
//		SPDLOG_ERROR("could not fork: {}", strerror(errno));
//		return -1;
//	}
...
//			usleep(250*1000);
//		}
//		SPDLOG_ERROR("sub-process did not complete in 5 seconds: {}", path);
//		return -1;
//	}
...
//	fd = open(procfn,1);
//	if (fd < 0) {
//        SPDLOG_ERROR("Can not open file {} error: {}", procfn, strerror(errno));
//        return 2;
//    }
...
//	if (pf == NULL) {
//		SPDLOG_ERROR("GGSN: DNS servers: error: could not open /etc/resolv.conf");
//		return 0;
//	}
//	char buf[300];
...
//				SPDLOG_ERROR("GGSN: Error: Invalid IP address in /etc/resolv.conf at: {}", buf);
//				continue;
//			}
//			dns[nfnd++] = addr;
//			if (nfnd == 2) break;
...
//	if (!strchr(copy,'.')) {
//		SPDLOG_WARN("Invalid GGSN.DNS option ignored: '{}'", copy);
//		return 0;
//	}
//	char *argv[3];
//	int argc = cstrSplit(copy,argv,3);						// split into argv
//	if (argc > 2) {
//		SPDLOG_WARN("GGSN: invalid GGSN.DNS option, more than 2 servers specified: '{}'", sdns.c_str());
...
//			// failed.
//			SPDLOG_ERROR("GGSN: unrecognized GGSN.DNS option: {}", sdns.c_str());
//			return i;
//		}
//	}
//	return argc;
