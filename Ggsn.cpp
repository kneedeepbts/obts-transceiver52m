...
void *miniGgsnReadServiceLoop(void *arg)
{
	Ggsn *ggsn = (Ggsn*)arg;
	sethighpri();
    // FIXME: Can't find where gBTS is initialized, so bypassing
	//while (ggsn->active() && !gBTS.btsShutdown()) {
    while (ggsn->active() && true) {
		struct pollfd fds[1];
		fds[0].fd = tun_fd;
		fds[0].events = POLLIN;
...
void *miniGgsnShellServiceLoop(void *arg)
{
    ...
    //std::string shname = gConfig.getStr(SQL_SHELLSCRIPT); // "GGSN.ShellScript"
    std::string shname = "";
    // FIXME: Can't find where gBTS is initialized, so bypassing
	//while (ggsn->active() && !gBTS.btsShutdown()) {
    while (ggsn->active() && true) {
...
bool Ggsn::start()
{
    ...
    // FIXME: Bypassing the dumb config system...
    //if (gConfig.getStr(SQL_SHELLSCRIPT).size() > 1) { // "GGSN.ShellScript"
	if (std::string("").size() > 1) {
