...
int configGetNumQ(const char *name, int defaultvalue)
{
    ...
    //        For now, we'll just pass the default value back.  Really need to
    //        review all the places this function is used and bypass them.
...
int configGprsChannelsMinCn() { return 0; }
int configGprsChannelsMinC0() { return 2; }
...
int configGprsMultislotMaxUplink() { return 2; }
int configGprsMultislotMaxDownlink() { return 3; }
...
{
	// BEGINCONFIG
	// 'GPRS.Enable',1,0,0,'Enable GPRS service: 0 or 1.  If enabled, GPRS service is advertised in the C0T0 beacon,
	// and GPRS service may be started on demand.  See also GPRS.Channels.*'
	// ENDCONFIG
    // FIXME: Bypassing the dumb config system...
    //        Skipping for now.  Will definitely want to enable GPRS soon.
...
bool GPRSConfig::sgsnIsInternal()
{
    ...
    // FIXME: Bypassing the dumb config system...
    //        Would probably be a good idea to look at making SGSN an external
    //        service (microservice) permanently.
    //        Forcing SGSN internal for now.
...
//	macDownlinkPersist = gConfig.getNum("GPRS.Downlink.Persist");	// (pat) We dont use this.
    macMSIdleMax = 600 * RLCBlocksPerSecond;
    macChIdleMax = 6000 * RLCBlocksPerSecond;
    macChCongestionMax = 60 * RLCBlocksPerSecond;
    // database number specified in percent:
    macChCongestionThreshold = 200 / 100.0;
    macDownlinkPersist = 0;	// (pat) We dont use this.
...
//	macUplinkKeepAlive = gConfig.getNum("GPRS.Uplink.KeepAlive");
    macDownlinkKeepAlive = 300;
    macUplinkPersist = 4000;
    macUplinkKeepAlive = 300;
	...
	for (usf = USFMIN; usf <= USFMAX; usf++) {
		...
		if (ms) {
			...
			RN_MS_FOR_ALL_TBF(ms,tbf) {
				...
				// An interesting point is that it might get its acknack before
