...
int ChannelHistory::getAvgRxlev()
{
    ...
    return round(ComputeTrend(mSCellData,now,&SCellPoint::getFrameNum,&SCellPoint::getRxlev,6));
}
...
bool NeighborHistory::nhGetAvgRxlev(FrameNum fn, float &avg, int algorithm)
{
    ...
    avg = ComputeTrend(nhList,fn,&NCellPoint::getFrameNum,&NCellPoint::getRxlev,6,algorithm);
	return true;
}
