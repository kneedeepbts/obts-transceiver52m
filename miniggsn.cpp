...
//	ggConfig.mgIpTimeout = gConfig.getNum(SQL_IP_TIMEOUT); "GGSN.IP.ReuseTimeout"
//	ggConfig.mgMaxPduSize = gConfig.getNum(SQL_PDU_MAX_SIZE); "GGSN.IP.MaxPacketSize"
//	ggConfig.mgMaxConnections = gConfig.getNum(SQL_PDP_MAX_COUNT); "GGSN.MS.IP.MaxCount"
//	ggConfig.mgIpTossDup = gConfig.getNum(SQL_IP_TOSS_DUP); "GGSN.IP.TossDuplicatePackets"
    ggConfig.mgIpTimeout = 180;
    ggConfig.mgMaxPduSize = 1520;
    ggConfig.mgMaxConnections = 254;
    ggConfig.mgIpTossDup = 0;

    // FIXME: Bypassing the dumb config system...
	//string logfile = gConfig.getStr(SQL_LOG_FILE); "GGSN.Logfile.Name"
    string logfile = "";
	if (logfile.length()) {
    ...
    // FIXME: Bypassing the dumb config system...
	//const char *ip_base_str = gConfig.getStr(SQL_IP_BASE).c_str(); "GGSN.MS.IP.Base"
    const char *ip_base_str = "192.168.99.1"; // FIXME: WHY?!
	...
	string route_save;
    // FIXME: Bypassing the dumb config system...
//	if (gConfig.defines(SQL_IP_ROUTE)) { // "GGSN.MS.IP.Route"
...
//	}
    ...
    // FIXME: Bypassing the dumb config system...
	//if ((firewall_enable = gConfig.getNum(SQL_FIREWALL_ENABLE))) { // "GGSN.Firewall.Enable"
    ...
    // FIXME: Bypassing the dumb config system...
	//const char *tun_if_name = gConfig.getStr(SQL_TUN_IF_NAME).c_str(); // "GGSN.TunName"
    const char *tun_if_name = "sgsntun";
