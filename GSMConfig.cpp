...
void GSMConfig::regenerateBeacon()
{
    ...
    if (neighbors.size()==0) neighbors.push_back(51);
...
template <class ChanType> ChanType* getChan(vector<ChanType*>& chanList, bool forGprs)
{
    ...
    // FIXME: Bypassing the dumb config system
    //        GSM.Channels.Randomize isn't a default setting, so just skipping
//	if (gConfig.defines(configRandomize)) { // "GSM.Channels.Randomize"
//		if (forGprs) {
...
void GSMConfig::createCBCH(ARFCNManager *radio, TypeAndOffset type, int CN, int TN)
{
    ...
    mCBCHDescription = L3ChannelDescription(type,TN,2,51+CN);
