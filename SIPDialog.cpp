	...
	if (sipmsg->smGetCode() == 0) {
		if (sipmsg->isINVITE()) {
			...
			if (mtLastResponse.smIsEmpty()) {
                ...
                if (! false) { MTSMSSendTrying(); }	// Otherwise just ignore the duplicate MESSAGE.
			} else {
...
SipDialog *SipDialog::newSipDialogMT(DialogType dtype, SipMessage *req)
{
	...
	if (proxy.empty()) {
        ...
        proxy = dtype == SIPDTMTSMS ? "127.0.0.1:5063" : "127.0.0.1:5060";
	}
...
SipDialog *SipDialog::newSipDialogRegister1() 		// caller imsi
{
    ...
    SipDialog *dialog = new SipDialog(SIPDTRegister,"127.0.0.1:5064","SIP.Proxy.Registration");
	...
	const string &contentType)
{
    ...
    SipDialog *dialog = new SipDialog(SIPDTMOSMS,"127.0.0.1:5063","SIP.Proxy.SMS");
